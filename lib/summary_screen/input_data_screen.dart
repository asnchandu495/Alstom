...
import 'package:qandaapp/summary_screen/input_data_screen_pressure.dart';
...
class InputDataScreen extends StatefulWidget {
  ...
  InputDataScreen(this._replicaBottomSheetValueSelection,
      this._mapOfValuesSelection, this._voidCallback, this._resetCallback,
      {Key? key})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => InputDataScreenState();
}

class InputDataScreenState extends State<InputDataScreen> {
  final String _nextTag = "Next";
  final String _repeatTag = 'Repeat';
  final String _assetIdTag = "Asset Identification number";
  final String _pressureTag = "Pressure Details";
  ...
  FocusNode _assetIdFocusNode = FocusNode();
  FocusNode _pressureFocusNode = FocusNode();
  FocusNode _temperatureFocusNode = FocusNode();

  final Rx<TextEditingController> _assetIdController =
      Rx(TextEditingController());
  final Rx<TextEditingController> _pressureController =
      Rx(TextEditingController());
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Obx(() => Padding(
                                ...
                                child: TextField(
                                  controller: _assetIdController.value,
                                  focusNode: _assetIdFocusNode,
                                  keyboardType: TextInputType.number,
                                  decoration: InputDecoration(
                                    enabled: true,
                                    // helperText: 'Say Confirm to move next',
                                    labelText: _assetIdTag,
  ...
  _moveToNextScreen(BuildContext context) {
    _stopEverything();
    widget._mapOfValuesSelection[_assetIdTag] =
        _assetIdTag + '-' + _assetIdController.value.text;

    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
            builder: (_) => InputDataScreenPressure(
  ...
  Widget _addToggle(int indexMain, String title, Options options) {
    ...
    onDetectedValues = (recognizedValue) async {
      ...
      if (recognizedValue.toLowerCase() == _readyTag.toLowerCase()) {
        isFound = false;
        _makeRepeatWork();
      } else if (_assetIdFocusNode.hasFocus && !isCommandFound) {
        ...
        try {
          ...
          isFound = true;
          _assetIdController.value.text = recognizedValue;
          // _pressureFocusNode.requestFocus();
          SpeechRecognizer().counter = 0;
          /*FocusScope.of(context).unfocus();*/

          var toSpeak = StringUtils.addCharAtPosition(recognizedValue, " ", 1, repeat: true);
          _speakAndListen(
              "You have provided $toSpeak, Say Next to confirm");
          // await TTSUtils().speak("You have selected $toSpeak");
        ...
        if (groupButtonController.selectedIndex == 0 ||
          ...
          holder.label = title;
          holder.value = value;
          // var toSpeak = StringUtils.addCharAtPosition(value, " ", 1, repeat: true);
  ...
  void _makeRepeatWork() {
    _speakAndListen(_assetIdTag);
    _assetIdController.value.text = '';
    // setState(() {
    FocusScope.of(context).requestFocus(_assetIdFocusNode);
    // });
  }
