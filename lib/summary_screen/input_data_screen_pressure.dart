...
import 'input_data_screen_temperature.dart';

class InputDataScreenPressure extends StatefulWidget {
  ...
  InputDataScreenPressure(this._replicaBottomSheetValueSelection,
      this._mapOfValuesSelection, this._voidCallback, this._resetCallback,
      {Key? key})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => InputDataScreenPressureState();
}

class InputDataScreenPressureState extends State<InputDataScreenPressure> {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Obx(() => Padding(
                                ...
                                child: TextField(
                                  controller: _pressureController.value,
                                  focusNode: _pressureFocusNode,
                                  keyboardType: TextInputType.number,
                                  decoration: InputDecoration(
                                      enabled: true,
                                      // helperText: 'Say Confirm to move next',
                                      labelText: _pressureTag,
  ...
  _moveToNextScreen(BuildContext context) {
    _stopEverything();
    widget._mapOfValuesSelection[_pressureTag] =
        _pressureTag + '-' + _pressureController.value.text;

    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
            builder: (_) => InputDataScreenTemperature(
  ...
  Widget _addToggle(int indexMain, String title, Options options) {
    ...
    onDetectedValues = (recognizedValue) async {
      ...
      if (recognizedValue.toLowerCase() == _readyTag.toLowerCase()) {
        isFound = false;
        _makeRepeatWork();
      } else if (_pressureFocusNode.hasFocus && !isCommandFound) {
        ...
        try {
          ...
          isFound = true;
          _pressureController.value.text = recognizedValue;
  ...
  void _makeRepeatWork() {
    _speakAndListen(_pressureTag);
    _pressureController.value.text = '';
    // setState(() {
    FocusScope.of(context).requestFocus(_pressureFocusNode);
    // });
  }
}
